    ...
    def createLabEnvironment(self):
        ...
        for line in labLines:
            ...
            if lineFixed == "[full]":
                ...
                if fullClone:
                    newVMID = self.pmconnection.create_clone_full(targetNode, lineFixed, targetPool)
                    self.pmconnection.set_network_bridge(targetNode, newVMID, "net0", targetNetwork)
                    if self.checkBox.isChecked():
                        self.pmconnection.startVM(newVMID, targetNode)
                else:
                    newVMID = self.pmconnection.create_clone_linked(targetNode, lineFixed, targetPool)
                    self.pmconnection.set_network_bridge(targetNode, newVMID, "net0", targetNetwork)
                    if self.checkBox.isChecked():
                        self.pmconnection.startVM(newVMID, targetNode)
